      ...
      ColumnLayout {
        ...
        Repeater {
          ...
          delegate: Item {
            ...
            readonly property bool translateToast: {
              var a = String(cardSlot.app || "")
              var s = String(cardSlot.summary || "")
              return a === "austraz.translate" || s === "Translation" || s === "Translation copied"
            }
            // Stock Omarchy: 5–8s floor, 30s cap, critical stays forever.
            // Translation only: 3s per line via expireTimeout, or sticky if critical.
            readonly property real lifetime: {
              if (cardSlot.translateToast) {
                if (cardSlot.urgency === 2)
                  return 0
                var ms = Number(cardSlot.expireTimeout || 0)
                if (isFinite(ms) && ms > 0)
                  return ms
              }
              return service.durationFor(cardSlot.urgency, cardSlot.expireTimeout)
            }
